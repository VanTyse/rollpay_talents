...
import { ProjectWithBalance } from "@/lib/types"
...
export default function ProjectsTable({ projects }: Props) {
  const headerItems: HeaderItem<ProjectWithBalance>[] = [
    {
      label: "Project",
      sortableBy: false,
...
function MobileRowItem({ project }: { project: ProjectWithBalance }) {
  return (
...
function RowItem(project: ProjectWithBalance) {
  return (
    <tr className="[&>td]:py- border-b border-rp-grey-1600 bg-white text-xs last-of-type:border-none hover:cursor-pointer [&>td]:px-6 [&>td]:py-4">
      ...
      <td className="text-sm font-semibold text-rp-grey-1700">
